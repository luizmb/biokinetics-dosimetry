...
/// as applied to internal-dosimetry biokinetic compartmental models.
///
...
///
/// The generic building blocks live in the `RungeKutta` package:
/// - ``Math/Matrix`` for the linear-algebra carrier;
/// - ``Math/Matrix/squared(times:)`` for the repeated-squaring step;
/// - ``Calculus/Taylor/exponential(of:tolerance:maxIterations:)`` for the
///   exponential's Taylor series of the *scaled* matrix.
///
/// This namespace assembles them with the threshold and conventions Birchall
/// proposed for compartmental dosimetry. If/when a non-Birchall scaling-and-squaring
/// variant (Padé approximant, Moler & Van Loan algorithm 11, etc.) is needed in
/// pure numerical linear algebra, it lands in the `Calculus` module of the
/// `RungeKutta` package under its own algorithm namespace.
///
...
/// 3. **Taylor-expand `e^Aₛ`** via ``Calculus/Taylor/exponential(of:tolerance:maxIterations:)``.
...
/// `1e-12` over 1000 days of integration — see `UraniumGoldenTests`.
///
/// References:
